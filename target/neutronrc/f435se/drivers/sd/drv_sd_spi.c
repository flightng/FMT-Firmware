...
const static struct sd_ops dev_ops = {
    .sd_init = init, .sd_write = sd_write, .sd_read = sd_read, .sd_control = sd_control};

rt_err_t drv_sd_init(const char* spi_dev_name) {
  rt_device_t spi_dev;
  ...
  init_spi_to_sdcard(spi_dev);
  sd0_dev.ops = &dev_ops;
  RT_TRY(rt_mutex_init(&sd_lock, "sd_lock", RT_IPC_FLAG_PRIO));
  return hal_sd_register(&sd0_dev, "sd0", RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_REMOVABLE | RT_DEVICE_FLAG_STANDALONE,
                         NULL);
}
