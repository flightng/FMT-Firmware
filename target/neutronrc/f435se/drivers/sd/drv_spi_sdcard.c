...
void spi_WriteByte_sd(rt_device_t device, uint8_t data) {
  rt_spi_transfer((struct rt_spi_device*)device, &data, RT_NULL, 1);
}

uint8_t spi_ReadByte_sd(rt_device_t device) {
  uint8_t data;
  rt_spi_transfer((struct rt_spi_device*)device, RT_NULL, &data, 1);
  return data;
}
...
SD_Error SD_Init(void)
{
  ...
  for (i = 0; i <= 9; i++)
  {
    ...
    spi_WriteByte_sd(spi_dev,SD_DUMMY_BYTE);
  } 
  ...
  if (!SD_GetResponse(SD_RESPONSE_NO_ERROR))
  {
    ...
    if (!SD_GetResponse(SD_START_DATA_SINGLE_BLOCK_READ))
    {
      ...
      for (i = 0; i < BlockSize; i++)
      {
        ...
        *pBuffer = spi_ReadByte_sd(spi_dev);
       
        /*!< Point to the next location where the byte read will be saved */
        pBuffer++;
      }
      /*!< Get CRC bytes (not really needed by us, but required by SD) */
      spi_ReadByte_sd(spi_dev);
      spi_ReadByte_sd(spi_dev);
...
SD_Error SD_WriteBlock(uint8_t* pBuffer, uint64_t WriteAddr, uint16_t BlockSize)
{
  ...
  if (!SD_GetResponse(SD_RESPONSE_NO_ERROR))
  {
    ...
    spi_WriteByte_sd(spi_dev,0xFE);

    /*!< Write the block data to SD : write count data by block */
    for (i = 0; i < BlockSize; i++)
    {
      /*!< Send the pointed byte */
      spi_WriteByte_sd(spi_dev,*pBuffer);
      /*!< Point to the next location where the byte read will be saved */
      pBuffer++;
    }
...
SD_Error SD_WriteMultiBlocks(uint8_t* pBuffer, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  while (NumberOfBlocks--)
  {
    ...
    spi_WriteByte_sd(spi_dev,SD_DUMMY_BYTE);
    /*!< Send the data token to signify the start of the data */
    spi_WriteByte_sd(spi_dev,SD_START_DATA_SINGLE_BLOCK_WRITE);
    /*!< Write the block data to SD : write count data by block */
    for (i = 0; i < BlockSize; i++)
    {
      /*!< Send the pointed byte */
  ...
  if (!SD_GetResponse(SD_RESPONSE_NO_ERROR))
  {
    if (!SD_GetResponse(SD_START_DATA_SINGLE_BLOCK_READ))
    {
      for (i = 0; i < 16; i++)
      {
        ...
        CSD_Tab[i] = spi_ReadByte_sd(spi_dev);
      }
    }
    /*!< Get CRC bytes (not really needed by us, but required by SD) */
    spi_WriteByte_sd(spi_dev,SD_DUMMY_BYTE);
...
SD_Error SD_GetCIDRegister(SD_CID* SD_cid)
{
  ...
  if (!SD_GetResponse(SD_RESPONSE_NO_ERROR))
  {
    if (!SD_GetResponse(SD_START_DATA_SINGLE_BLOCK_READ))
    {
      ...
      for (i = 0; i < 16; i++)
      {
        CID_Tab[i] = spi_ReadByte_sd(spi_dev);
      }
    }
    /*!< Get CRC bytes (not really needed by us, but required by SD) */
    spi_WriteByte_sd(spi_dev,SD_DUMMY_BYTE);
...
void SD_SendCmd(uint8_t Cmd, uint32_t Arg, uint8_t Crc)
{
  ...
  for (i = 0; i < 6; i++)
  {
    spi_WriteByte_sd(spi_dev,Frame[i]); /*!< Send the Cmd bytes */
  }
}
...
uint8_t SD_GetDataResponse(void)
{
  ...
  {
    /*!< Read resonse */
    response = spi_ReadByte_sd(spi_dev);
    /*!< Mask unused bits */
    response &= 0x1F;
    switch (response)
    {
  ...
  /*!< Wait null data */
  while (spi_ReadByte_sd(spi_dev) == 0);
...
SD_Error SD_GetResponse(uint8_t Response)
{
  ...
  while ((spi_ReadByte_sd(spi_dev) != Response) && Count)
  {
    Count--;
  }
  if (Count == 0)
...
uint16_t SD_GetStatus(void)
{
  ...
  Status = spi_ReadByte_sd(spi_dev);
  Status |= (uint16_t)(spi_ReadByte_sd(spi_dev) << 8);
...
SD_Error SD_GetCardType(void)
{
  ...
  /*!< Check if response is got or a timeout is happen */
  while (( (R1_Resp = spi_ReadByte_sd(spi_dev)) == 0xFF) && Count)
  {
    Count--;
  }
  if (Count == 0)
  ...
  else if (R1_Resp == SD_IN_IDLE_STATE)
  {
      ...
      for (i = 0; i < 4; i++)
      {
        R7R3_Resp[i] = spi_ReadByte_sd(spi_dev);
      }
...
void init_spi_to_sdcard(rt_device_t dev) { spi_dev = dev; }
/**
  * @}
  */
